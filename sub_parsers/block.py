    ...
    def __init__(self, parser: Parser, source_file: str):
        ...
        while (
            ...
            if parser.current_token(source_file).type == "N_FUNC":
                ...
                if parser.current_token(source_file).value in (
                    "heading1",
                    "heading2",
                    "heading3",
                    "heading4",
                    ...
                    if p.level == "1" and heading1_registered:
                        raise Exception(
                            f"Error: heading1 (h1) is already registered in {source_file}. If you repeat it you will have a bad score!"
                        )
                    elif p.level == "1" and not heading1_registered:
                        heading1_registered = True

                    if past_heading and past_heading not in (
                        int(p.level),
                        int(p.level) + 1,
                        int(p.level) - 1,
                    ):
                        raise Exception(
                            "Error: if you add multiple headings they have to be in order. Do not skip headings!"
                        )
                    else:
